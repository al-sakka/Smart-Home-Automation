...
{
	if (a_currentFlameVal == LOGIC_HIGH)
	{
		if (*a_prevFlameVal == LOGIC_LOW)
		{
			LCD_clearScreen();
		}
	...
	{
		if (*a_prevFlameVal == LOGIC_HIGH)
		{
			LCD_clearScreen();
			LCD_ShowInfo();
		}

		LCD_ShowData(a_motorState, a_temp, a_ldr);
	}

	*a_prevFlameVal = a_currentFlameVal;
}
